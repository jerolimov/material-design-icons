...
for folder in action alert av communication content device editor file hardware image maps navigation notification social toggle; do
    ...
    for image in $(find "$folder/ios" -iname '*_black_*pt.png'); do
    ...
    for image in $(find "$folder/ios" -iname '*_amber_*pt.png'); do
        basename=$(basename $image)
        echo "<tr><td><img src=$image></td><td>$basename</td></tr>"
    done >> $folder.html
    ...
    for image in $(find "$folder/ios" -iname '*_red_*pt.png'); do
        basename=$(basename $image)
        echo "<tr><td><img src=$image></td><td>$basename</td></tr>"
    done >> $folder.html
    ...
    for image in $(find "$folder/ios" -iname '*_white_*pt.png'); do
        basename=$(basename $image)
        echo "<tr><td><img src=$image></td><td>$basename</td></tr>"
    done >> $folder.html
